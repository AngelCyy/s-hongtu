(function(root){
	...
	function SvgNest(){
			...
			p.map(function(pair){
				...
				p2.map(worker.placePaths).then(function(placements){
					...
					if(!best || bestresult.fitness < best.fitness){
						...
						var gNode = self.applyPlacement(best.placements)[0].children;
						for(var i = 0;i<gNode.length;i++){
                            console.log(gNode[i].offsetLeft)
                        }
			...
			function toTree(list, idstart){
				...
				for(i=0; i<list.length; i++){
					...
					for(j=0; j<list.length; j++){
						...
						// console.log(GeometryUtil.pointInPolygon(p[0], list[j]),p[0], list[j],i,j)
		...
		this.applyPlacement = function(placement,isNewPath){
			var i, j, k;
			var clone = [];
			...
			for(i=0; i<placement.length; i++){
				...
				for(j=0; j<placement[i].length; j++){
					...
					if(isNewPath){
                        var trans = 't'+p.x+','+p.y+'r'+p.rotation;
                        var nowD = clone[part.source].getAttribute('d');
                        console.log(Raphael.toMatrix(nowD,trans));
            			var pathStrB = Raphael.mapPath(nowD,Raphael.toMatrix(nowD,trans)).toString();
                        console.log(nowD,pathStrB);

                        // clone[part.source].setAttribute('d',pathStrB);
                        clone[part.source].setAttribute('transform','translate('+p.x+' '+p.y+') rotate('+p.rotation+')');
