	...
	$.svgIcons = function(file, opts) {
		var svgns = "http://www.w3.org/2000/svg",
			...
			if(true || opts.svgz) {
				...
				var data_el = $('<object data="' + file + '" type=image/svg+xml>').hide();
		...
		function getIcons(evt, no_wait) {
			...
			if(!opts.no_img) {
				...
				.load(function () {
					...
					makeIcons(true, opts.fallback);
				}).error(function () {
					makeIcons(false , opts.fallback);
				});
			} else {
				setTimeout(function() {
					if(!icons_made) makeIcons(false , opts.fallback);
				},500);
			}
		}
		...
		function makeIcons(toImage, fallback) {
			...
			if(fallback) {
				...
				$.each(fallback, function(id, imgsrc) {
                    ...
                    if (imgsrc == 'flyouth.png')
					{
                        var icon = $(new Image())
                            .attr({
                                //'class': 'image',//'svg_icon',
                                src: path + imgsrc,
                                'width':5,//icon_w,
                                'height': 5,//icon_h,
                                'alt': 'icon'
						...
						console.log('===============================test: ' + icon_w);
					}
                    else if (imgsrc == 'dropdown.gif')
                    {
                        var icon = $(new Image())
                            .attr({
                                //'class': 'image',//'svg_icon',
                    ...
                    else if (imgsrc == 'new_layer.png' ||
                        	  imgsrc == 'delete_layer.png' ||
                        	  imgsrc == 'rename_layer.png' ||
                        	  imgsrc == 'up_layer.png' ||
						      imgsrc == 'down_layer.png' ||
                        	  imgsrc == 'lock_layer.png' ||
							  imgsrc == 'show_layer.png')
					{
						if (imgsrc ==  'show_layer.png')
						{
                            var icon = $(new Image())
                                .attr({
                                    src: path + imgsrc,
				...
				for(var i = 0; i < len; i++) {
					...
					///*
					// Extract icon from svgs
					var vsize = parseInt(svg.getAttribute('viewBox').split(' ')[3]);
                    var escale = 64 / vsize;
					saveSvgAsPng(svg , id + ".png" , {scale:escale});
					//*/
				}
