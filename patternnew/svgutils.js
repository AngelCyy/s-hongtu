...
{
	if (path.pathSegList == null || path.pathSegList.numberOfItems == 0)
	{
		return null;
	}
	...
	return {
		'x': bbox.cx - bbox.width * 0.5,
		'y': bbox.cy - bbox.height * 0.5,
		'width': bbox.width,
		'height': bbox.height
	};
...
svgedit.utilities.getIsolatedPaths = function(singlePaths)
{
	if (singlePaths)
	{
		console.log('==================================== input path nums: ' + singlePaths.length);

		// Get all intersected segments
		var getIntersectedSegs = function(_singlePaths)
		{
			/*
			var seg =
			{
				id : 0,
				p0: {x : 0 , y : 0 , connected : false},
				p1: {x : 0 , y : 0 , connected : false},
				path : '',
			};
			*/

			var interSegs = [];
			var interSegCounter = 0;

			for (var i = 0 ; i < _singlePaths.length ; ++i)
			{
                var allPts = new Array();

				for (var j = 0 ; j < _singlePaths.length ; ++j)
				{
					if (i != j)
					{

                        var path1 = extendPath(_singlePaths[i],false,10);

                        var path2 = extendPath(_singlePaths[j],false,10);

                        // var OOrt = Raphael.pathIntersection(_singlePaths[i] , _singlePaths[j]);

                        var oldrt = Raphael.pathIntersection(path1 , path2);

                        var rt = delRepeatRts(oldrt);

                        // console.log("想看看格式是怎么样的",rt)
                        if(rt.length < 1)
                        {
                            // console.log("就没有进入判断端点的模式,提取裁片====")

                            var pathdata1 = getPointData(_singlePaths[i]);

                            var pathdata2 = getPointData(_singlePaths[j]);

                            if(isEqual(pathdata1.start,pathdata2.start) || isEqual(pathdata1.start,pathdata2.end)){

                                rt.push(pathdata1.start);
                        ...
                        rt = delRepeatRts(rt);
                        // console.log("拿到的交点比较1==",rt,oldrt,OOrt);

                        for (var k = 0 ; k < rt.length ; ++k)
						{
                            var cp = closestPoint(_singlePaths[i] , rt[k]);

                            allPts.push(cp.length);

                            //console.log('=--==push length: ' + cp.length);
						}
					}
				}
                ...
                // Sort all intersect pts;
                allPts.sort(sortNumber);

                // console.log(allPts);

                // console.log('==========inte pts: ' + allPts.length);
                    ...
                    interSegs.push(
                        {
                            id : interSegCounter++,
							srcId : i,
                            p0 : Raphael.getPointAtLength(_singlePaths[i], allPts[k]) ,
                            p1 : Raphael.getPointAtLength(_singlePaths[i], allPts[k + 1]) ,
							p0connected : false ,
							p1connected : false ,
                            path : Raphael.getSubpath(_singlePaths[i] , allPts[k] , allPts[k + 1]),
                        }
                    )
                }
			}
		...
		var isPointOverlapped = function(p0 , p1)
		{
			var distance = Math.sqrt((p0.x - p1.x) * (p0.x - p1.x) + (p0.y - p1.y) * (p0.y - p1.y));

			// console.log('=================== check distance: ' + distance);
			return distance <= 10;
        };

		var intersectedSegs = getIntersectedSegs(singlePaths);

		// console.log("拿到的焦点数据3==",intersectedSegs);

        var getNextSeg = function(srcSegIdx)
        {
        	// Consider self-intersection
            for (var i = 0 ; i < intersectedSegs.length ; ++i)
            {
                if (intersectedSegs[i].id != srcSegIdx && intersectedSegs[i].srcId != intersectedSegs[srcSegIdx].srcId)
                {
                    if (intersectedSegs[i].p0connected == false)
                    {
                        if (intersectedSegs[srcSegIdx].p0connected == false)
                        {
                            if(isPointOverlapped(intersectedSegs[i].p0 , intersectedSegs[srcSegIdx].p0))
                            {
                                intersectedSegs[i].p0connected = true;
                                intersectedSegs[srcSegIdx].p0connected = true;
                                return i;
                            }
                        }

                        if (intersectedSegs[srcSegIdx].p1connected == false)
                        {
                            if(isPointOverlapped(intersectedSegs[i].p0 , intersectedSegs[srcSegIdx].p1))
                            {
                                intersectedSegs[i].p0connected = true;
                                intersectedSegs[srcSegIdx].p1connected = true;
                                return i;
                            }
                        }
                    }

                    if (intersectedSegs[i].p1connected == false)
                    {
                        if (intersectedSegs[srcSegIdx].p0connected == false)
                        {
                            if(isPointOverlapped(intersectedSegs[i].p1 , intersectedSegs[srcSegIdx].p0))
                            {
                                intersectedSegs[i].p1connected = true;
                                intersectedSegs[srcSegIdx].p0connected = true;
                                return i;
                            }
                        }
                        ...
                        if (intersectedSegs[srcSegIdx].p1connected == false)
                        {
                            if(isPointOverlapped(intersectedSegs[i].p1 , intersectedSegs[srcSegIdx].p1))
                            {
                                intersectedSegs[i].p1connected = true;
                                intersectedSegs[srcSegIdx].p1connected = true;
                                return i;
            ...
            for (var i = 0 ; i < intersectedSegs.length ; ++i)
            {
                if (intersectedSegs[i].id != srcSegIdx)
                {
                    if (intersectedSegs[i].p0connected == false)
                    {
                        if (intersectedSegs[srcSegIdx].p0connected == false)
        ...
        {
            for (var i = 0; i < intersectedSegs.length; ++i) {
                intersectedSegs[i].p0connected = false;
                intersectedSegs[i].p1connected = false;
            }
        }

		console.log('inter seg num: ' + intersectedSegs.length);

		if (intersectedSegs && intersectedSegs.length >= singlePaths.length)
		{
			for (var i = 0 ; i < intersectedSegs.length ; ++i)
			{
                resetConnection();
				...
				var startSegIdx = i;
				var iteSegIdx = getNextSeg(startSegIdx);

				//console.log('next: ' + iteSeg.id);

				var temPath = [startSegIdx];

				// console.log('---start id : ' + startSegIdx);
				while (iteSegIdx != -1 && iteSegIdx != startSegIdx)
				{
                    console.log('---000 iteSeg id : ' + iteSegIdx + ', ' + intersectedSegs[iteSegIdx].p0connected + ', ' + intersectedSegs[iteSegIdx].p1connected);

					temPath.push(iteSegIdx);
                    var nextSegIdx = getNextSeg(iteSegIdx);
                    console.log('---111 iteSeg id : ' + iteSegIdx + ', ' + intersectedSegs[iteSegIdx].p0connected + ', ' + intersectedSegs[iteSegIdx].p1connected);

                    iteSegIdx = nextSegIdx;
				}

				if (iteSegIdx == startSegIdx)
				{
					// Bingo! Find isolated path, return it
					var isolatedPaths = [];
					for (var k = 0 ; k < temPath.length ; ++k)
					{
                        isolatedPaths.push(intersectedSegs[temPath[k]].path);
                        /*
                        var sp = svgCanvas.addSvgElementFromJson({
                            element: 'path',
                            attr: {
                                d: intersectedSegs[temPath[k]].path,
                                fill: 'none' ,
								stroke: 'blue',
                            }
                        });
                        ...
                        console.log(sp);
                        console.log('status: ' + intersectedSegs[temPath[k]].p0connected + ', ' + intersectedSegs[temPath[k]].p1connected);
                     */
					}
