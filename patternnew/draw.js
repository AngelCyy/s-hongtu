...
svgedit.draw.Drawing.prototype.getLayerColor = function (i) {
	if (i >= 0 && i < this.getNumLayers())
	{
		if (this.all_layers[i][1].hasAttribute('layercolor') == false)
		{
            this.all_layers[i][1].setAttribute('layercolor' , svgCanvas.fetchLayerColor());
		}

		return this.all_layers[i][1].getAttribute('layercolor');
	}
...
svgedit.draw.Drawing.prototype.getCurrentLayerColor = function()
{
	if (this.current_layer != null)// && this.current_layer[1] != undefined)
	{
        return this.current_layer.getAttribute('layercolor');
	}

	return '';
};

svgedit.draw.Drawing.prototype.getCurrentLayerElement = function() {
	return this.current_layer[1];
};

svgedit.draw.Drawing.prototype.getCurrentLayerLocked = function()
{
	...
	var isLocked = (this.current_layer.getAttribute('locked') == 'true');

    console.log('========================== is layer locked: ' + isLocked);
...
*/
svgedit.draw.Drawing.prototype.createLayer = function(name , color) {
	var svgdoc = this.svgElem_.ownerDocument;
	var new_layer = svgdoc.createElementNS(NS.SVG, "g");
	new_layer.setAttribute('layercolor' , color);
	var layer_title = svgdoc.createElementNS(NS.SVG, "title");
...
svgedit.draw.Drawing.prototype.getLayerLocked = function(layername) {
	// find the layer
	var layer = null;
	var i;
	for (i = 0; i < this.getNumLayers(); ++i) {
	...
	if (!layer) {return false;}
	return (layer.getAttribute('locked') == 'true');
};
/**
...
svgedit.draw.Drawing.prototype.setLayerLocked = function(layername, bLocked) {
	...
	layer.setAttribute("locked", bLocked);
	return layer;
};
/**
