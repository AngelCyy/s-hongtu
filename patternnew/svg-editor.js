...
(function() {
    ...
    window.svgEditor = (function($) {
        ...
        editor.loadSvgString = loadSvgString = function(str, callback) {
            var svgc = svgCanvas.setSvgString2(str)
            var success = svgc !== false;
        ...
        editor.init = function ()
        {
            ...
            //*/
            console.log("===================以上是setIcon========================= init icons111");
            ...
            var setSelectMode = function(idx) {
                ...
                if (curr.length && curr[0].id !== 'tool_select') {
                    ...
                    $('#tool_select').addClass('tool_button_selected').removeClass('tool_button');
            ...
            var toolButtonClick = editor.toolButtonClick = function(button, noHiding) {
                ...
                $(button).addClass('tool_button_selected').removeClass('tool_button');
                return true;
            };
                ...
                if (ext.buttons) {
                    ...
                    $.each(ext.buttons, function(i, btn) {
                        ...
                        switch ( btn.type ) {
                            case 'mode_flyout':
                            case 'mode':
                                cls = 'tool_button toolLeft_list';
                                parent = '#headerTool_left';
                                break;
                            case 'context':
                                cls = 'tool_button';
                                parent = '#' + btn.panel;
                        ...
                        if (!btn.includeWith && !btn.list) {
                            if ('position' in btn) {
                                if ($(parent).children().eq(btn.position).length) {
                                    //插入 移动 工具的标签
                                    $(parent).children().eq(3).before(button);
                                }
                                else {
                            ...
                            if (btn.type =='mode_flyout') {
                                ...
                                if (!ref_btn.parent().hasClass('tools_flyout')) {
                                    ...
                                    show_btn = ref_btn.clone()
                                        ...
                                        .append($('<img>' , {'src':btn.icon,'width':'26','height':'26','alt':'icon'}));
...
//									key: btn.key,
            ...
            $('.fillstyle_item').mousedown(function(evt)
            {
                ...
                var defsElem = svgdocbox.createElementNS('', 'defs');
                defsElem.innerHTML = paletteFill[id];
            ...
            $('#svgcanvas').css('background' , '#333333');
            (function() {
                ...
                $('#svgcanvas').bind('mousemove mouseup', function(evt)
                {
                    ...
                    console.log("在这里调用了？-----",(evt.clientX - last_x) , (evt.clientY - last_y));
...
//					$('.contextMenu').hide();
			...
			*/
            ...
            {
                path.ctrlCallback(true);
            }

            var altCallback = function()
            {
                path.altCallback();
            }
...
//				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
                ...
                this.update = function(apply) {
                    ...
                    switch (selectedElement.tagName) {
                        ...
                        default:
                            ...
                            var defColor = type === 'fill' ? 'none' : '#fff';
            ...
            Actions = (function() {
                ...
                var tool_buttons = [
					...
					*/
                    ...
                    //{key: 'ctrl', fn: ctrlCallback} ,
                    {key: 'alt', fn: altCallback} ,
                    {key: 'shift', fn: shiftCallback} ,
                    {key: 'm' , fn: ctrlCallback},
                ];
            ...
            editor.ready(function() {
                ...
                $('#ruler_x').toggle(false);//!!curConfig.showRulers);
                $('#ruler_y').toggle(false);
                $('#ruler_corner').toggle(false);
            ...
            // $('#workarea').contextMenu(function(e){
            //     return false;
            //     });
             // {menu: 'PopupBox',
             //        inSpeed: 0}
                // function(){document.getElementById("test").oncontextmenu = function(e){
            // return false;
                //     alert("两点间的距离为："+distanceBetweenTwoPoint)
                // }
            ...
            editor.insertImageFromUrl = function(_imageUrl)
            {
                ...
                if (imageUrl != undefined)
                {
                    ...
                    if (contains(imageUrl , '.drf')||contains(imageUrl , '.dxf'))
            ...
            if (window.FileReader) {
                ...
                var open = $("#inp_import").change(function(e) {
                    var f = this;
                    editor.openPrep(function(ok)
                    {
                        ...
                        if (f.files.length === 1)
                        {
                            ...
                            reader.onloadend = function(e)
                            {
                                var newSvg = loadSvgString(e.target.result);
                                ...
                                $("#ImportGroup").append(ImportSubG);

                                var childs = $(newSvg).children();


                               if($(newSvg).find("g[layercolor='#FF4FFF']").length > 0){

                                   console.log("是本系统导出的啊========");

                                   var pathToAbsolute = function(pathStr){

                                       var abso = pathStr;

                                       if(pathStr.indexOf('m') >= 0)
                                       {
                                            console.log('这个是相对模式的路径',pathStr);
                                           // pathStr.toString().toUpperCase();
                                           // console.log('转化为大写的===',pathStr);
                                           var strArr = Raphael.parsePathString(pathStr)
                                           var pt = {x:parseFloat(strArr[0][1]),y:parseFloat(strArr[0][2])};
                                           console.log(strArr,pt);

                                           var newStr = new Array();

                                           newStr.push('M'+ pt.x +','+ pt.y);

                                           for(var i=1;i<strArr.length; i++)
                                           {
                                               var str = strArr[i];
                                                // console.log(str)
                                                if(str[0] == 'c' && str.length == 7)
                                                {
                                                    newStr.push('C')
                                                    newStr.push((parseFloat(str[1]) + pt.x)+ ',' + (parseFloat(str[2]) + pt.y));
                                                    newStr.push((parseFloat(str[3]) + pt.x)+ ',' + (parseFloat(str[4]) + pt.y));
                                                    newStr.push((parseFloat(str[5]) + pt.x)+ ',' + (parseFloat(str[6]) + pt.y));
                                                    pt = {x:parseFloat(str[5]) + parseFloat(pt.x),y:parseFloat(str[6]) + parseFloat(pt.y)};
                                                }
                                   ...
                                   var  resetPathAttr= function(childs){
                                       ...
                                       for(var i=0; i<childs.length; i++)
                                       {
                                           ...
                                           console.log("这里是循环========",child.tagName);

                                           switch (child.tagName){
                                               case 'g':
                                                   var childPaths = $(child).find('path');
                                                    console.log(childPaths)
                                                   if(childPaths.length > 0)
                                                   {
                                                       for(var j=0; j<childPaths.length; j++)
                                                       {
                                                           var pathd = childPaths[j].getAttribute('d');
                                                           var abso = pathToAbsolute(pathd);
                                                           childPaths[j].setAttribute('d',abso);
                                                       }
                                                   }
                                                   ...
                                                   break;
                                               case 'path':
                                                   var abso = pathToAbsolute(child);
                                                   child[i].setAttribute('d',abso);
                                                   break;
                                           }

                                           if(child.tagName == 'path') {

                                               console.log("难倒没有进入这里？");
                                ...
                                $(ImportSubG).append($(newSvg).children());
                      ...
                      /*  if (!ok) {return;}
                        ...
                        // var node = SvgCanvas.setSvgString2(e.target.result);
                        console.log("有么有到这里啊===");
                        ...
                        reader.onloadend = function(e) {
                            console.log("有么有到这里啊===",reader);

                            var file = e.target.files[0];

                            console.log("怎么到这里来了？-----",file);
    ...
    //                                 display.innerHTML = '';
                                    ...
                                    console.log("有么有拿到这个啊===",svg,childs);
                                    ...
                                    for (var i = 0; i < childs.length; i++) {
                                        var child = childs[i];
                                        svgedit.utilities.assignAttributes(child, {
                                            id:"open_"+i,
                                            stroke:"#fff",
                                            fill:'none'
                                        });
                                        ImportG.appendChild(child);
                                    }
                                }
                                catch(e){
                                    console.log(e);
