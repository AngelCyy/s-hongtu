...
var gripRadius = svgedit.browser.isTouch() ? 5 : 2;
	...
	this.selectorRect = this.selectorGroup.appendChild(
		svgFactory_.createSVGElement({
			...
			'attr': {
				...
				'fill': 'none',
				'stroke': svgCanvas.getCurrentDrawing().getCurrentLayerColor(),//'#22C',
				'stroke-width': '1',
...
svgedit.select.Selector.prototype.reset = function(e) {
	...
	this.resize(0);
	this.selectorGroup.setAttribute('display', 'none');
    // console.log('===================== show select');

    svgCanvas.showSelectedElement();
};
...
svgedit.select.Selector.prototype.resize = function(idx)
{
	//console.log('============================================ call idx: ' + idx);
	...
	//console.log('======================================== dstring: ' + dstr);
	selectedBox.setAttribute('d', dstr);
    selectedBox.setAttribute('stroke-width',  1 / svgCanvas.getCanvasScale());
    selectedBox.setAttribute('stroke-dasharray' , (5 / svgCanvas.getCanvasScale()) + ',' + (5 / svgCanvas.getCanvasScale()));
...
//	if (selected === selectedElements[0]) {
		...
		for (dir in this.gripCoords) {
            ...
            selectedGrips[dir].setAttribute('r',  gripRadius / svgCanvas.getCanvasScale());
		}
    	...
    	mgr.rotateGripConnector.setAttribute('stroke-width',  1 / svgCanvas.getCanvasScale());

		mgr.rotateGrip.setAttribute('cx', nbax + (nbaw)/2);
		mgr.rotateGrip.setAttribute('cy', nbay - (gripRadius*5));
    	mgr.rotateGrip.setAttribute('r', gripRadius / svgCanvas.getCanvasScale());
    	mgr.rotateGrip.setAttribute('stroke-width', 1.5  / svgCanvas.getCanvasScale());

    	mgr.centerGrip.setAttribute('cx', nbax + (nbaw)/2);
   	 	mgr.centerGrip.setAttribute('cy', nbay + (nbah)/2);
    	mgr.centerGrip.setAttribute('r', gripRadius / svgCanvas.getCanvasScale());
    	mgr.centerGrip.setAttribute('stroke-width', 1.5  / svgCanvas.getCanvasScale());
//	}
};
...
svgedit.select.SelectorManager.prototype.initGroup = function() {
    ...
    this.canvasForgeground = svgFactory_.createSVGElement({
        'element': 'g',
        'attr': {'id': 'canvasForgeground'}
    });
    ...
    this.crossRulerLines = svgFactory_.createSVGElement({
        'element': 'path',
        'attr': {
            'id': 'svgcrossrulerlines',
            'fill': 'none',
            'stroke': '#22C',
            'stroke-width': '1',
            'stroke-dasharray': '5,5',
    ...
    this.canvasOverlay.appendChild(this.crossRulerLines);

	this.erasorCursorRect = svgFactory_.createSVGElement({
		'element': 'rect',
		'attr': {
			'id': 'svgerasorcursorrect',
			'fill': '#fff',
			'stroke': '#fff',
			'stroke-width': '0',
			'style': 'pointer-events:none'
	...
	this.canvasOverlay.appendChild(this.erasorCursorRect);

	this.erasorCursorCircle = svgFactory_.createSVGElement({
		'element': 'circle',
		'attr': {
			'id': 'svgerasorcursorcircle',
			'fill': '#fff',
			'stroke': '#ffff',
			'stroke-width': '0',
			'style': 'pointer-events:none'
	...
	this.canvasOverlay.appendChild(this.erasorCursorCircle);

	this.setTransform = function(transString)
	{
		this.selectorParentGroup.setAttribute('transform' , transString);

        this.canvasForgeground.setAttribute('transform' , transString);
	}
	...
	*/
    ...
    this.selectorParentGroup.appendChild(this.canvasOverlay);
	...
	this.centerGrip = this.selectorGripsGroup.appendChild(
        svgFactory_.createSVGElement({
            'element': 'circle',
            'attr': {
                'id': 'selectorGrip_center',
                'fill': '#22C',
                'r': gripRadius,
                'stroke': '#22C',
                'stroke-width': 1
	...
	this.rotateGrip = this.selectorGripsGroup.appendChild(
		svgFactory_.createSVGElement({
			...
			'attr': {
				...
				'stroke': '#22C',
				'stroke-width': 1,
				'style': 'cursor:url(' + config_.imgPath + 'rotate.png) 12 12, default;'
			}
		})
	);
...
//	if (!svgedit.browser.isOpera()) rect.setAttribute('filter', 'url(#canvashadow)');
    ...
    function setupForgeground()
	{
		/*
        var rulerX = svgFactory_.createSVGElement({
            'element': 'path',
            'attr': {
                'id': 'ruler0',
                'fill': 'none',
                ...
                'style': 'pointer-events:none' ,
                'd' : 'M -10000 , 100  L 10000 , 100'
            }
        });

        selectorManager_.canvasForgeground.appendChild(rulerX);
		*/

        var selector = svgedit.utilities.getElem('selectorParentGroup');
        svgFactory_.svgRoot().insertBefore(selectorManager_.canvasForgeground , selector);
	}

    setTimeout(setupForgeground , 500);
};

svgedit.select.SelectorManager.prototype.setSelectorColor = function(_color)
{
	...
	for (i = 0; i < N; ++i)
	{
		if (this.selectors[i])
		{
			this.selectors[i].selectorRect.setAttribute('stroke' , _color);
		}
	}
};
...
svgedit.select.SelectorManager.prototype.getCrossRulerLines = function() {
	return this.crossRulerLines;
};

svgedit.select.SelectorManager.prototype.getErasorCursor = function() {
	return this.erasorCursor;
};
